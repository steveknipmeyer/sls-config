...
# =============================================================================
# SECTION 2b: /etc/sudoers.d
#
# Sudoers drop-in rules for the openclaw user. These are separate from the
# main /etc/sudoers file and are scoped to specific script paths only.
#
# openclaw-scripts — grants the openclaw user passwordless sudo for the two
#   sls-system audit scripts (security-audit.sh, system-health.sh). These
#   scripts need root to read /var/log/auth.log, query fail2ban-client, and
#   get full process/port visibility. Privilege is narrowly scoped — not
#   general sudo.
# =============================================================================
...
harvest_file "/etc/sudoers.d/openclaw-scripts" "${STATE_DIR}/etc/sudoers.d/openclaw-scripts"
...
| \`state/etc/sudoers.d/openclaw-scripts\` | Custom | Passwordless sudo for sls-system audit scripts (scoped to two paths) |
...
### Sudoers Rules (sls-system audit scripts)

The \`openclaw\` user has no general sudo privileges. A narrow drop-in rule at
\`/etc/sudoers.d/openclaw-scripts\` grants passwordless root access to exactly
two script paths used by the \`sls-system\` cron skill:

\`\`\`bash
# Create /etc/sudoers.d/openclaw-scripts with the harvested contents
sudo cp state/etc/sudoers.d/openclaw-scripts /etc/sudoers.d/openclaw-scripts
sudo chmod 0440 /etc/sudoers.d/openclaw-scripts
sudo visudo -c -f /etc/sudoers.d/openclaw-scripts

# Verify
sudo -l -U openclaw
\`\`\`

These scripts require root to read \`/var/log/auth.log\`, query \`fail2ban-client\`,
and obtain full process/port visibility. Using a scoped sudoers rule rather than
general sudo or elevated exec provides the minimum privilege necessary.
...
config/state/etc/sudoers.d/openclaw-scripts
