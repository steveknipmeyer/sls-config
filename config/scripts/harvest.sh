...
#         openclaw-gateway.env         — REDACTED: shared host gateway token env (OpenClaw + web server)
...
#   And: grep -r "REDACTED" /home/openclaw/.openclaw/projects/sls-config/config/state/etc/openclaw-gateway.env
...
harvest_file() {
    ...
    if [ ! -e "$src" ]; then
        log "  ✗ $src (missing — skipping)"
        echo "# HARVEST ERROR: Missing $src" > "$dest"
    elif [ -r "$src" ]; then
...
# ---
# openclaw-exec.sh — host-routed OpenClaw execution wrapper used by agents.
# Current model: thin /opt forwarder to shared skill script path.
# ---
harvest_file "/opt/openclaw-exec.sh" "${STATE_DIR}/opt/openclaw-exec.sh"
...
# openclaw-gateway.env — shared gateway token environment source loaded by
# both OpenClaw and sls-web-server services.
harvest_file "/etc/openclaw-gateway.env" "${STATE_DIR}/etc/openclaw-gateway.env"
redact_secrets "${STATE_DIR}/etc/openclaw-gateway.env"
...
config/state/etc/openclaw-gateway.env
