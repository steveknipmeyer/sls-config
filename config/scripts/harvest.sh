...
#   sudo bash /home/openclaw/.openclaw/workspace/extras/config/scripts/harvest.sh
#
#   Then commit as the openclaw user:
#   sudo -u openclaw git -C /home/openclaw/.openclaw/workspace add extras/config/
#   sudo -u openclaw git -C /home/openclaw/.openclaw/workspace commit -m "Steve: harvest snapshot $(date +%Y-%m-%d)"
#   sudo -u openclaw git -C /home/openclaw/.openclaw/workspace push
#
# OUTPUT STRUCTURE:
#   extras/config/
#     schemas/
...
#
#   IMPORTANT: Always review extras/config/state/ before committing to git.
#   Run: git -C /home/openclaw/.openclaw/workspace diff extras/config/state/
#   And: grep -r "REDACTED" extras/config/state/opt/openclaw.env  (verify redaction worked)
#
# =============================================================================
...
STATE_DIR="${WORKSPACE_ROOT}/extras/config/state"
SCHEMA_DIR="${WORKSPACE_ROOT}/extras/config/schemas"
SCRIPT_DIR="${WORKSPACE_ROOT}/extras/config/scripts"
...
\`extras/config/scripts/harvest.sh\` and should be committed to git after review.
...
# Expected: origin git@github.com:steveknipmeyer/ada.git

# If missing, add it
git -C /home/openclaw/.openclaw/workspace remote add origin git@github.com:steveknipmeyer/ada.git

# Clone fresh on a new deployment
git clone git@github.com:steveknipmeyer/ada.git /home/openclaw/.openclaw/workspace
...
The OpenClaw config schema is captured at \`extras/config/schemas/openclaw.schema.json\`
  ...
  "json.schemas": [
    {
      ...
      "url": "file:///home/openclaw/.openclaw/workspace/extras/config/schemas/openclaw.schema.json"
    }
  ]
}
\`\`\`
...
find "${STATE_DIR}" -type f | sed "s|${WORKSPACE_ROOT}/||" | sort > /tmp/harvest-actual.txt

cat << 'EXPECTED' | sort > /tmp/harvest-expected.txt
extras/config/state/README.md
extras/config/state/docker-images.txt
extras/config/state/openclaw-doctor.txt
extras/config/state/ufw-status.txt
extras/config/state/versions.txt
extras/config/state/etc/ssh/sshd_config
extras/config/state/home/openclaw/.bashrc
extras/config/state/home/openclaw/.ssh/config
extras/config/state/home/openclaw/dot-openclaw/openclaw.json
extras/config/state/home/openclaw/openclaw.code-workspace
extras/config/state/usr/local/bin/openclaw
extras/config/state/opt/openclaw.env
extras/config/state/opt/openclaw-cli.sh
extras/config/state/opt/openclaw-tui.sh
extras/config/state/opt/restart-openclaw.sh
extras/config/state/opt/rotate-openclaw-gateway.sh
extras/config/state/opt/setup-openclaw-domain.sh
extras/config/state/opt/status-openclaw.sh
extras/config/state/opt/tailscale-reauth.sh
extras/config/state/opt/update-openclaw.sh
extras/config/state/systemd/openclaw.service
extras/config/state/systemd/sls-web-server.service
extras/config/schemas/openclaw.schema.json
EXPECTED
...
if [ -z "$UNEXPECTED" ]; then
    ...
    log_warn "      rm /home/openclaw/.openclaw/workspace/<path>"
fi
...
while IFS= read -r file; do
    if grep -Pq "${SECRET_PATTERN}" "$file" 2>/dev/null; then
        log_warn "  ✗ POSSIBLE SECRET in: ${file#${WORKSPACE_ROOT}/}"
...
chown -R openclaw:openclaw "${WORKSPACE_ROOT}/extras/config"
chmod -R u+r "${WORKSPACE_ROOT}/extras/config"
log "  ✓ Ownership fixed: ${WORKSPACE_ROOT}/extras/config"
...
log ""
log "NEXT STEPS:"
log "  1. Review changes:  git -C ${WORKSPACE_ROOT} diff extras/config/"
...
log "  3. Commit (as openclaw user):"
log "     sudo -u openclaw git -C ${WORKSPACE_ROOT} add extras/config/"
log "     sudo -u openclaw git -C ${WORKSPACE_ROOT} commit -m 'Steve: harvest snapshot ${DATE_ONLY}'"
log "     sudo -u openclaw git -C ${WORKSPACE_ROOT} push"
log ""
